...
export const getNurse = async (req, res) => {
  try {
    const _users = await dbquery("SELECT * FROM TBL_NURSE", []);
...
export const createNurse = async (req, res) => {
  try {
    ...
    const _user = await dbquery(
      "SELECT * FROM TBL_NURSE WHERE EMAIL = ? OR USERNAME = ?",
      [email, username],
    );
    ...
    const _createUser = await dbquery(
      "INSERT INTO TBL_NURSE ( FULL_NAME, USERNAME, PASSWORD, EMAIL, PERMITTED_SERVICE ) VALUES ( ?,?,?,?,? )",
